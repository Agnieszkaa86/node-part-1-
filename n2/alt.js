...
const logFile = path.join(_dirname, program.opts().file);
const logFileDirectory = path.dirname(logFile);//obcina nazwe pliku ze sciezki
...
fs.access(logFile)
    ...
    .catch(() => {
        fs.mkdir(logFileDirectory, { recursive: true })
            ...
            .catch(() => {
                console.log("Nie udało się stworzyć katalogów".red);
                process.exit(1)
            });
    }); 